...
describe('fetchEpisodesForChannel', () => {
  ...
  beforeEach(() => {
    api.episodes.fetchForChannel.mockClear();
  });

  it('should fetch all episodes for a channel', () => {

    api.episodes.fetchForChannel.mockImplementation(() => {
      return new Promise(resolve => {
        resolve(mockPayload);
      });
    });
    ...
    return store
      ...
      .then(() => {
        expect(api.episodes.fetchForChannel).toBeCalledWith(1, 1, 'test');
        const actions = store.getActions();
        expect(actions.length).toBe(2);
        expect(actions[0].type).toBe(FETCH_EPISODES_REQUEST);
        expect(actions[1].type).toBe(FETCH_EPISODES_SUCCESS);
